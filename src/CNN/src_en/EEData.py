...
class EEData:
    ...
    def __init__(self,data_dir,split = None,label2id_file = "../trigger_data_en/label2id.dat"):
    ...
    def load_from_files(self,ids_file,sent_file,golden_file):
        ...
        for line in open(ids_file):
            ...
            if len(line) !=13:
                ...
                print len(line)
                continue
            doc_id,sent_id,token_id,offset,length,train_label,token = line[:7]
            token = token.strip()
            if token in string.punctuation:
                continue
            if not train_label in self.label2id:
                train_label = "other"
            if self.is_train:
